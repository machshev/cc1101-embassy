...
An async [Embassy][embassy] driver for the [CC1101](cc1011) sub-1 GHz RF transceiver, built on [`embedded-hal`](embedded-hal) 1.0 and [`embedded-hal-async`](embedded-hal) 1.0.
...
The CC1101 uses SPI mode 0 (CPOL=0, CPHA=0) at up to 10 MHz.
The example below uses SPI0 on a Raspberry Pi Pico — adjust pin numbers to match your wiring.
...
> **Note:** The E07-M1101D modules from Ebyte have an SMA connector and include an antenna.
Fit the antenna before testing — range and sensitivity are significantly worse without it.
